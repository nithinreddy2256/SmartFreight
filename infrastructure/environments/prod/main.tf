...
locals {
  ...
  microservices = {
    shipment-service     = { port = 8080, cpu = 1024, memory = 2048, desired = 3 }
    carrier-service      = { port = 8081, cpu = 1024, memory = 2048, desired = 3 }
    invoice-service      = { port = 8082, cpu = 2048, memory = 4096, desired = 2 }
    document-service     = { port = 8083, cpu = 1024, memory = 2048, desired = 2 }
    notification-service = { port = 8084, cpu = 512, memory = 1024, desired = 2 }
    analytics-service    = { port = 8085, cpu = 2048, memory = 4096, desired = 2 }
  }
}
...
module "kms" {
  ...
  alias_name              = "main"
  description             = "SmartFreight prod environment master KMS key"
...
# ============================================================
# VPC (3 NAT Gateways for prod HA)
# ============================================================
module "vpc" {
  source = "../../modules/vpc"
...
# ============================================================
# ALB (Internal for prod microservices)
# ============================================================
module "alb" {
  source = "../../modules/alb"
...
# ============================================================
# Aurora Clusters (Provisioned Multi-AZ for prod)
# ============================================================
module "aurora_shipment_db" {
  ...
  allowed_security_group_ids = [
    module.ecs_service["shipment-orchestrator"].security_group_id,
    module.ecs_service["carrier-integration"].security_group_id,
  ]
...
module "s3_documents" {
  ...
  lifecycle_expiration_days = 2555 # 7 years for compliance
  kms_key_arn               = module.kms.key_arn
  force_destroy             = false
  depends_on                = [module.kms]
}
...
module "cognito" {
  ...
  logout_urls                = ["https://app.smartfreight.com/logout"]
  resource_server_identifier = "https://api.smartfreight.com"
